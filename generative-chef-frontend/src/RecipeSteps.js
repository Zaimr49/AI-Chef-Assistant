    ...
    const fetchImages = async () => {
      ...
      try {
        const promises = steps.map((step) =>
          axios
            .post(
              process.env.REACT_APP_HUGGINGFACE_MODEL_URL,
              {
                inputs: step,
              },
              {
  ...
  return (
    <div>
      ...
      {steps.map((step, index) => (
        <div
          key={index}
          className="card mt-3 bg-dark text-white border border-muted d-flex justify-content-center align-items-center"
          style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)", height: "300px" }}
        >
          <div className="card-body text-center">
            ...
            {loading ? (
              ...
              <>
                <img
                  src={images[index] || "placeholder-image-url"}
