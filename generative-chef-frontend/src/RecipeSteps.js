      ...
      {steps
        ...
        .map((pair, rowIndex) => (
          <div key={rowIndex} className="row">
            {pair.map((step, colIndex) => (
              <div key={colIndex} className="col-md-6">
                <div
                  className="card mt-3 bg-dark text-white border border-muted"
                  style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)" }}
                >
                  <div className="card-body">
                    <h5 className="card-title">
                      Step {rowIndex * 2 + colIndex + 1}
                    </h5>
                    <p className="card-text">{step}</p>
                    {loading ? (
                      <>
                      ...
                      <>
                        <img
                          src={
                            images[rowIndex * 2 + colIndex] ||
                            "placeholder-image-url"
                          }
                          alt={`Visualization for step ${
